...
public partial class AdminActiveDistributerReport : System.Web.UI.Page
{
    ...
    public string getattendance()
    {
        ...
        using (cmd = new SqlCommand())
        {
            ...
            cmd.CommandText = "sp_AdminActiveDistributerReport";
            ...
            using (sdr = cmd.ExecuteReader())
            {
                if (sdr.HasRows)
                {
                    while (sdr.Read())
                    {
                        ...
                        string compsize = Convert.ToString(sdr[12]);
                        string region = Convert.ToString(sdr[15]);
                        string country = Convert.ToString(sdr[14]);
                        string city = Convert.ToString(sdr[13]);

                        data += "<tr><td>" + sno + "</td><td>" + "<img src='" + logo + "' width='85' height='35' />" + "</td><td>"
                            + name + "</td><td>" + email + "</td><td>"
                            + phone + "</td><td>" + website + "</td><td>" + contactpersonname + "</td><td>" + contactpersonemail + "</td><td>" + contactpersonphone + "</td><td>" +
