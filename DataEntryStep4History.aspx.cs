...
public partial class DataEntryStep4History : System.Web.UI.Page
{
    Fetch fet = new Fetch();
    ...
    public string getattendance()
    {
        ...
        using (cmd = new SqlCommand())
        {
            ...
            cmd.CommandText = "sp_DataEntryStep4";
            ...
            using (sdr = cmd.ExecuteReader())
            {
                if (sdr.HasRows)
                {
                    while (sdr.Read())
                    {
                        ...
                        string createdon = Convert.ToDateTime(sdr[6]).ToLongDateString() + " " + Convert.ToDateTime(sdr[6]).ToShortTimeString();
                        string reason = Convert.ToString(sdr[7]);
                        string user = Convert.ToString(sdr[8]);
                        data += "<tr><td>" + sno + "</td><td>" + inword + "</td><td>" + noofnewid + "</td><td>"
                            + noofsameid + "</td><td>" + noofmodification + "</td><td>" + discription + "</td><td>" + createdon + "</td><td>"
                            + reason + "</td><td>" + user + "</td></tr>";
                    }
                }
            }
            return data;
